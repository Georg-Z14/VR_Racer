...
STREAM_BACKEND = os.getenv("STREAM_BACKEND", "mediamtx").strip().lower()
MEDIAMTX_WEBRTC_URL = os.getenv("MEDIAMTX_WEBRTC_URL", "").strip()
...
class CameraManager:
    def __init__(self, relay: MediaRelay, target_size=(1280, 720), sensitivity=40):
        ...
        from app.camera_stream import CameraProcess
    ...
    def get_tracks(self, vr_mode: bool):
        ...
        from app.camera_stream import StereoSharedMemoryCameraStream
...
KEY_FILE = os.getenv("KEY_FILE", "data/secret.key")
...
async def stream_config(request: web.Request) -> web.Response:
...
async def camera_second(request: web.Request) -> web.Response:
    ...
    if STREAM_BACKEND == "mediamtx" or camera_manager is None:
...
async def offer(request: web.Request) -> web.Response:
    ...
    if STREAM_BACKEND == "mediamtx":
        return web.Response(status=410, text="Python WebRTC stream disabled; using MediaMTX")
...
def create_app() -> web.Application:
    ...
    init_db()
    if STREAM_BACKEND != "mediamtx" and camera_manager is None:
    ...
    app.router.add_get("/stream/config", stream_config)
...
def main():
    ...
    ssl_context = None
    protocol = "http"
    if _parse_bool(os.getenv("HTTPS_ENABLED", "0"), False):
        cert_file = os.getenv("HTTPS_CERT_FILE", "certs/vrracer.crt")
        key_file = os.getenv("HTTPS_KEY_FILE", "certs/vrracer.key")
        ssl_context = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)
        ssl_context.load_cert_chain(cert_file, key_file)
        protocol = "https"
    print(f"🌍 Lokaler Server: {protocol}://0.0.0.0:{port}")
